...
# define Length (float)40 // lenght of one block
# define fpss (float)(70.0)
# define g (float)(9.8)
# define pi (float)3.1415926535
# define shocknum 5
# define warmth (float)2.0
# define beta (float) 360

# define threshold (float)0.01
# define slop (float)0.01
...
float click_perception(int mouse_x, int mouse_y, Object * A);
...
void one_term_with_earthquake(Object A[], int NumberOfObject, cinf contact[], int earthquake_type, float time_duration, float *time, float amp, float frequency);
...
int main(int argc, char **argv) {
	...
	window = SDL_CreateWindow("Blueprint", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 1360, 768, SDL_RENDERER_ACCELERATED || SDL_WINDOW_OPENGL);
	...
	while (!quit) {
		...
		//							SDL_DestroyTexture(wood_triangle_block);
									...
									for (i = 0; i < 4 * population; i++) {
										contact[i].run = false;
										contact[i].normal_impulse[0] = 0;
										contact[i].normal_impulse[1] = 0;
										contact[i].tangent_impulse[0] = 0;
										contact[i].tangent_impulse[1] = 0;
									}
								}
			...
			if (stage_start == true && quit == false) {
				time = 0;
				float timestart = 10;
				float timeduration = 50;
				float accumulator = 0;
				float framestart= SDL_GetPerformanceCounter();
				float currenttime;
				...
				while (quit == false) {
					...
					accumulator = accumulator + (currenttime - framestart)/(SDL_GetPerformanceFrequency());
					framestart= SDL_GetPerformanceCounter();

					initial_accumulate = accumulator;
					fpsstep[0] = false; fpsstep[1] = false; fpsstep[2] = false;

					if (accumulator > 1.0) accumulator = 1.0;

					while (accumulator>(1/fpss)*0.25) {
						if (time < timestart) {
							one_term(object, population, contact);
							time = time + 1.0 / fpss;
						}
						else if (time < timeduration && time>timestart) one_term_with_earthquake(object, population, contact, 1, timeduration, &time, 10, 0.2);
						else one_term(object, population, contact);
						accumulator = accumulator - (1 / fpss)*0.25;
						if ((accumulator < 0.75*initial_accumulate && fpsstep[0] == false) || (accumulator<0.5*initial_accumulate&&fpsstep[1]==false)||(accumulator<0.25*initial_accumulate&&fpsstep[2]==false)) {
							for (i = 0; i < population; i++) {
								if (object[i].shape->number == 1) {
									stretchTextureEx(renderer, object[i].x[0] - Length / 2, object[i].x[1] - Length / 2, Length, Length, wood_one_block, object[i].th * 180 / pi);
								}
							...
							drawTexture(renderer, 0, 0, map);
							drawTexture(renderer, 1004, 0, right_menu);
							if (fpsstep[0] == false) fpsstep[0] = true;
							else if (fpsstep[1] == false) fpsstep[1] = true;
							else if (fpsstep[2] == false) fpsstep[2] = true;
						}
					}
...
void cycle2(Object A[], int n, int NumberOfObject, cinf contact[]) { // A[n]
	int i;
...
void one_term(Object A[], int NumberOfObject, cinf contact[]) {
	...
	for (j = 0; j < 10; j++) { 
		...
		for (i = 0; i < NumberOfObject; i++) {
			cycle2(A, i, NumberOfObject, contact);
		}
		for (i = 0; i < NumberOfObject; i++) {
			A[i].v[0] = A[i].v[0] + A[i].delv[0];
			A[i].v[1] = A[i].v[1] + A[i].delv[1];
...
//	for (i = 0; i < shocknum; i++) shock_propagation(A, NumberOfObject); iteration = 0;
...
/*	int i;
	...
	int i,j;
	int index;
	float * depth_array; depth_array = (float *)malloc((A->shape->vertex_num)*sizeof(float));
	float best_depth=-DBL_MAX;
	float a_vertex[2]; // vertex coordinate o f A about origin
	float b_vertex[2]; // vertex coordinate of B about origin
...
void find_impulse(Object * A, Object * B, int NumberOfObject, cinf contact[], int numA,  int numB) { // Let B sink to the A
	...
	float impulse; // Impulse magnitude
...
/*				P = rB[0] * rB[1] * B->inv_inertia + rA[0] * rA[1] * A->inv_inertia;
				Q = rB[1] * rB[1] * B->inv_inertia + rA[1] * rA[1] * A->inv_inertia + A->inv_mass + B->inv_mass;
				R = rB[0] * rB[0] * B->inv_inertia + rA[0] * rA[0] * A->inv_inertia + A->inv_mass + B->inv_mass;
				imx = ((1 + e)*(B->v[1] - A->v[1])*P - (1 + e)*(B->v[0] - A->v[0])*R)/(P*P-Q*R);
				imy= ((1 + e)*(B->v[1] - A->v[1])*Q + (1 + e)*(B->v[0] - A->v[0])*P) / (P*P - Q*R);
				impulse = -sqrt(imx*imx + imy*imy);*/

				contact_num = 0;
				for (i = 0; i < 4 * NumberOfObject; i++) if (contact[i].run == true) if ((contact[i].numA == numA&&contact[i].numB == numB) || (contact[i].numB == numA&&contact[i].numA == numB)) contact_num = contact_num + 1;
				impulse = impulse / contact_num;
				impulse = impulse + beta*(contact[index].depth + slop);

				if (impulse + v_dot(contact[index].normal_impulse, A->shape->normal[indexA]) < 0) {
					contact[index].normal_impulse[0] = contact[index].normal_impulse[0] + impulse*A->shape->normal[indexA][0];
					contact[index].normal_impulse[1] = contact[index].normal_impulse[1] + impulse*A->shape->normal[indexA][1];
				}
				else{
					impulse = v_magnitude(contact[index].normal_impulse);
					contact[index].normal_impulse[0] = 0;
					contact[index].normal_impulse[1] = 0;
				}
...
void find_fraction(Object * A, Object * B, int NumberOfObject, cinf contact[], int numA, int numB) { // Let B sink to the A
	...
	float impulse; // Impulse magnitude
	float tempt_vector[2]; // temporary vector for calculus
	float VA[2]; // velocity of collision point about A (next value)
	float rA[2]; // position vector of collision point from COM of A
	float VB[2]; // velocity of collision point about B (next value)
	float rB[2]; // position vector of collision point from COM of B
	float VBA[2]; // VB-VA
	int indexA, indexB;
	int index;
	...
	for (indexB = 0; indexB < B->shape->vertex_num; indexB++) {
		if (B->shape->vertex_contact_index[indexB] != -1 && contact[B->shape->vertex_contact_index[indexB]].numA == numA) {
			...
			if (v_dot(VBA, tempt_vector) != 0) {
				...
				float fraction_magnitude = u*impulse;
				float fraction_vector[2];
				v_mult(tempt_vector, fraction_magnitude, fraction_vector);
				float calculate_tempt[2], calculate_tempt2[2];
				...
				if (v_magnitude(VBA) > v_magnitude(fraction_vector)*A->inv_mass + v_magnitude(fraction_vector)*B->inv_mass + v_magnitude(calculate_tempt)*B->inv_inertia + v_magnitude(calculate_tempt2)*A->inv_inertia) {
					contact_num = 0;
					for (i = 0; i < 4 * NumberOfObject; i++) if (contact[i].run == true) if ((contact[i].numA == numA&&contact[i].numB == numB) || (contact[i].numB == numA&&contact[i].numA == numB)) contact_num = contact_num + 1;
					fraction_vector[0] = fraction_vector[0] / contact_num;
					fraction_vector[1] = fraction_vector[1] / contact_num;
					float delwB, delwA; // delta value of angular velocity of B and A
					float delvA[2], delvB[2]; // delta value of COM velocity of A and B
					delwB = -v_cross(rB, fraction_vector)*B->inv_inertia;
					delwA = v_cross(rA, fraction_vector)*A->inv_inertia;
					v_mult(fraction_vector, -B->inv_mass, delvB);
					v_mult(fraction_vector, A->inv_mass, delvA);
				...
				contact[index].tangent_impulse[0] = contact[index].tangent_impulse[0]+ fraction_vector[0];
				contact[index].tangent_impulse[1] = contact[index].tangent_impulse[1]+ fraction_vector[1];
			}
//			}
		}
	}
...
float click_perception(int mouse_x, int mouse_y, Object * A) { // find the pedetration depth of B about A: B sink to the A
	int i;
	float depth;
	float best_depth = -DBL_MAX;
	float a_vertex[2]; // vertex coordinate o f A about origin
...
void shock_propagation(Object A[], int NumberOfObject) {
	...
	float best_y_value=-DBL_MAX;
	float last_y_value = DBL_MAX;
...
void find_shock(Object * A, Object * B, bool * contact) { // Let B sink to the A
	...
	float rB[2]; // position vector of collision point from COM of B
	float VBA[2]; // VB-VA
	float imaginary_inv_mass_A = A->inv_mass;
	float imaginary_inv_mass_B = B->inv_mass;
	float imaginary_inv_inertia_A = A->inv_inertia;
	float imaginary_inv_inertia_B = B->inv_inertia;
	int indexA, indexB;
	...
	for (i = 0; i < NumberOfObject; i++) {
		for (j = 0; j < A[i].shape->vertex_num; j++) {
			if (A[i].shape->vertex_contact_index[j] != -1) {
				...
				indexA = contact[index].indexA;
				A[numB].delv[0] = A[numB].delv[0] - contact[index].normal_impulse[0] * A[numB].inv_mass*0.5;
				A[numB].delv[1] = A[numB].delv[1] - contact[index].normal_impulse[1] * A[numB].inv_mass*0.5;
				A[numA].delv[0] = A[numA].delv[0] + contact[index].normal_impulse[0] * A[numA].inv_mass*0.5;
				A[numA].delv[1] = A[numA].delv[1] + contact[index].normal_impulse[1] * A[numA].inv_mass*0.5;
				A[numB].delw = A[numB].delw - v_cross(contact[index].rB, contact[index].normal_impulse)* A[numB].inv_inertia*0.5;
				A[numA].delw = A[numA].delw + v_cross(contact[index].rA, contact[index].normal_impulse)* A[numA].inv_inertia*0.5;
				contact[index].normal_impulse[0] = 0;
				contact[index].normal_impulse[1] = 0;

				A[numB].delv[0] = A[numB].delv[0] - contact[index].tangent_impulse[0] * A[numB].inv_mass*0.5;
				A[numB].delv[1] = A[numB].delv[1] - contact[index].tangent_impulse[1] * A[numB].inv_mass*0.5;
				A[numA].delv[0] = A[numA].delv[0] + contact[index].tangent_impulse[0] * A[numA].inv_mass*0.5;
				A[numA].delv[1] = A[numA].delv[1] + contact[index].tangent_impulse[1] * A[numA].inv_mass*0.5;
				A[numB].delw = A[numB].delw - v_cross(contact[index].rB, contact[index].tangent_impulse)* A[numB].inv_inertia*0.5;
				A[numA].delw = A[numA].delw + v_cross(contact[index].rA, contact[index].tangent_impulse)* A[numA].inv_inertia*0.5;
				contact[index].tangent_impulse[0] = 0;
				contact[index].tangent_impulse[1] = 0;
			}
		}
	}
}
...
void one_term_with_earthquake(Object A[], int NumberOfObject, cinf contact[], int earthquake_type, float time_duration, float *time, float amp, float frequency) {
	...
	*time = *time + 1.0 / fpss;
	...
	for (i = 0; i < NumberOfObject; i++) if (A[i].material.number == 99) { A[i].v[0] = 0; A[i].v[1] = 0; }
}

void earthquake(Object * A, float time, int type, float amp, float frequency) {
	enum ShakeFuntion { cosine = 1, sine, updowncosine, updownsine, cosine_updonwcosine};

	if (type == cosine) A->v[0] = amp*cos(time*2*pi*frequency);
	else if (type == sine) A->v[0] = amp*sin(time*2*pi*frequency);
	else if (type ==updowncosine) A->v[1] = amp*cos(time * 2 * pi*frequency);
	else if(type==updownsine) A->v[1] = amp*sin(time * 2 * pi*frequency);
	else if (type == cosine_updonwcosine) { A->v[0] = amp*cos(time * 2 * pi*frequency); A->v[1] = amp*cos(time * 2 * pi*frequency); }
}

void map_create(int stage, int step, Object * A, int * population, float * height, float * timeduration, int* earthquake_type, float* amp, float* frequency) {
	if (stage == 1) {
		if (step == 1) {
			*height = 500;
			*timeduration = 50;
			*population = 1;
			A = (Object*)malloc(sizeof(Object)*(*population));
